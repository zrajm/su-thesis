...
\textbf{Notera:} Du behöver ladda mallen innan du kan sätta variablerna. Notera
också att inte alla variabler sätts med ett språkargument. Dessa språkfria
variabler är "\texttt{author}", "\texttt{supervisor}" och
"\texttt{frontinfo}".
...
%% If 'frontinfo' is set, it replaces the 'department', 'thesistype', 'course',
%% 'program', 'semester' and 'supervisor' values with a free-form title page
%% text (you may include su-thesis values here using '\suget').
\suset{frontinfo}{%
  © 2020--2022 av zrajm, Uppsala \\
  English title: \suget[english]{title}
}

%% Swedish metadata (only used if language is Swedish, and 'frontinfo' isn't set)
...
%% English metadata (only used if language is English, and 'frontinfo' isn't set)
...
\begin{table}
  ...
  \label{tab-ipa}
  \vspace{.5em}
  \input{tables/svenska-konsonanter}
  \vspace{1em}
\end{table}
